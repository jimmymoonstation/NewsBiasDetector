(function () {
  ...
  function injectFloatingUI() {
    ...
    // Optional: keep click-to-toggle if needed
    floatingButton.addEventListener('click', togglePanel);

    // Automatically open panel and fetch GPT data
//    openPanel();
//    fetchData('gpt');
  ...
  function injectFloatingPanel() {
    if (document.getElementById('ext-floating-panel')) return;
    ...
    floatingPanel.innerHTML = `
      <div class="ext-panel-content">
        ...
        <h1 class="ext-model-display" id="ext-title">AI News Evaluator</h1>

        <div class="ext-model-display" id="ext-model"></div>
  ...
  function openPanel() {
    if (!floatingPanel) injectFloatingPanel();
    isPanelOpen = true;
  ...
  async function fetchData(model) {
    try {
      ...
      document.getElementById('ext-result').innerText = `Failed: ${error.message}`;
    }
  }
  ...
  new MutationObserver(() => {
    ...
    if (url !== lastUrl) {
      ...
      setTimeout(() => {
        injectFloatingUI();
        fetchData('gpt'); // refetch on URL change
      }, 1000);
    }
